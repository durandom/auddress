  ...
  class Google
    ...
    def post(url, body, headers)
      if @in_batch
        ...
        raise FetchingError.new(response, body.to_s) unless response.is_a? Net::HTTPSuccess
    ...
    def get_contact(id)
      response = response_body self.get(id, {})
      c = Contact.new(self, response)
    end
    ...
    class Base
      ...
      def id=(str)
        @xml.at('entry').inner_html += "<id/>" unless @xml.at('id')
        @xml.at('id').inner_html = str
      end
      ...
      def create!
        ...
        xml = gmail.response_body response
        @xml = Hpricot(xml.to_s, :xml => true)
        response
      end
      ...
      def reload!
        raise "Cannot reload new entry" if new?
        contact = gmail.get_contact(self.id)
        @xml = contact.xml
        self
      end
    ...
    class Contact < Base
      ...
      def initialize(gmail, xml = nil)
        ...
        if xml.nil?
          ...
          @xml.root.inner_html += PRIMARY_EMAIL_TAG
        end
      ...
      # replaces the fields
      def replace_fields(name, fields)
        # first remove all fields
        @xml.search("gd:#{name}").remove
        # now add them at the beginning
        @xml.at('entry').inner_html += fields.class == String ? fields : fields.to_html
      end
