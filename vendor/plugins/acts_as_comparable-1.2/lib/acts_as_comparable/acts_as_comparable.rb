  ...
  module InstanceMethods
    ...
    def different?( other = nil, options = nil )
      ...
      comparable_attributes_for( options ).each_pair do |attr, action|
        if action.is_a? Proc
          return true unless action.call( self ).to_s == action.call( other ).to_s
        else
          return true unless self.send( attr ).to_s == other.send( attr ).to_s
        end
      end
      false
    end
    ...
    def differences( other = nil, options = nil )
      ...
      diffs = {}
      comparable_attributes_for( options ).each_pair do |attr, action|
        if action.is_a? Proc and (a=action.call( self ).to_s) != (b=action.call( other ).to_s)
          diffs[attr] = [ a, b ]
        elsif not action.is_a? Proc and (a=self.send( attr ).to_s) != (b=other.send( attr ).to_s)
          diffs[attr] = [ a, b ]
        end
      end
